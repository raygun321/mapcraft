 ...
 * @author rmalot
 */
public class MainControllerSaved {
    ...
    private void handleMouse(Node scene, final Node root) {
        scene.setOnMousePressed((event) -> {    
            ...
            if(res != null) {
                ...
                System.out.println("IntersectedNode: " + res.getIntersectedNode());
                System.out.println("IntersectedFace: " + res.getIntersectedFace());
                
                if(res.getIntersectedFace() > 0) {
                    Chunk theChunk = chunkManager.getChunkByNode(res.getIntersectedNode());
                    theChunk.getBlockByFace(res.getIntersectedFace()).setActive(false);
                    theChunk.update();
                }
            }
        });
